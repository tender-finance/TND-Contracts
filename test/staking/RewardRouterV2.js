...
describe("RewardRouterV2", function () {
  // const config
  const bnMultiplierToDistributor = 10000;
  ...
  beforeEach(async () => {
    ...
    await bonusTndDistributor.setBonusMultiplier(bnMultiplierToDistributor);
    ...
    // Other
    await stakedTndDistributor.setTokensPerInterval(
      esTndPerIntervalToDistributor
    );

    await bnTnd.setMinter(wallet.address, true);
    await bnTnd.mint(bonusTndDistributor.address, bnTndToDistributor);

    await esTnd.setMinter(wallet.address, true);
    await esTnd.mint(stakedTndDistributor.address, esTndToDistributer);
  ...
  it("inits", async () => {
    ...
    // Set private
    expect(await esTnd.inPrivateTransferMode()).eq(true);
    expect(await stakedTndTracker.inPrivateTransferMode()).eq(true);
    expect(await stakedTndTracker.inPrivateStakingMode()).eq(true);
    expect(await bonusTndTracker.inPrivateTransferMode()).eq(true);
    expect(await bonusTndTracker.inPrivateStakingMode()).eq(true);
    expect(await bonusTndTracker.inPrivateClaimingMode()).eq(true);
    expect(await feeTndTracker.inPrivateTransferMode()).eq(true);
    expect(await feeTndTracker.inPrivateStakingMode()).eq(true);

    // Set handler and minter
    expect(await stakedTndTracker.isHandler(rewardRouter.address)).eq(true);
    expect(await stakedTndTracker.isHandler(bonusTndTracker.address)).eq(true);

    expect(await bonusTndTracker.isHandler(rewardRouter.address)).eq(true);
    expect(await bonusTndTracker.isHandler(feeTndTracker.address)).eq(true);
    expect(await bonusTndDistributor.bonusMultiplierBasisPoints()).eq(
      bnMultiplierToDistributor
    );

    expect(await feeTndTracker.isHandler(rewardRouter.address)).eq(true);
    expect(await feeTndTracker.isHandler(tndVester.address)).eq(true);

    expect(await esTnd.isHandler(rewardRouter.address)).eq(true);
    expect(await esTnd.isHandler(stakedTndTracker.address)).eq(true);
    expect(await esTnd.isHandler(stakedTndDistributor.address)).eq(true);
    expect(await esTnd.isHandler(tndVester.address)).eq(true);
    expect(await esTnd.isMinter(tndVester.address)).eq(true);

    expect(await bnTnd.isHandler(feeTndTracker.address)).eq(true);
    expect(await bnTnd.isMinter(rewardRouter.address)).eq(true);

    expect(await tndVester.isHandler(rewardRouter.address)).eq(true);

    // Other
    expect(await stakedTndDistributor.tokensPerInterval()).eq(
      esTndPerIntervalToDistributor
    );

    expect(await bnTnd.isMinter(wallet.address)).eq(true);
    expect(await bnTnd.balances(bonusTndDistributor.address)).eq(
      bnTndToDistributor
    );

    expect(await esTnd.isMinter(wallet.address)).eq(true);
    expect(await esTnd.balances(stakedTndDistributor.address)).eq(
      esTndToDistributer
    );

    expect(await tndVester.isHandler(wallet.address)).eq(true);
  });
  ...
  it("stakeTndForAccount, stakeTnd, stakeEsTnd, unstakeTnd, unstakeEsTnd, claimEsTnd, claimFees, compound, batchCompoundForAccounts", async () => {
    ...
    await expect(
      rewardRouter
        .connect(user0)
        .stakeTndForAccount(user1.address, expandDecimals(1000, 18))
    ).to.be.revertedWith("Governable: forbidden");

    await rewardRouter.setGov(user0.address);
    await rewardRouter
      .connect(user0)
      .stakeTndForAccount(user1.address, expandDecimals(800, 18));
    ...
    await tnd
      .connect(user1)
      .approve(stakedTndTracker.address, expandDecimals(200, 18));
    ...
    expect(
      await stakedTndTracker.depositBalances(user0.address, tnd.address)
    ).eq(0);
    expect(await stakedTndTracker.stakedAmounts(user1.address)).eq(
      expandDecimals(1000, 18)
    );
    expect(
      await stakedTndTracker.depositBalances(user1.address, tnd.address)
    ).eq(expandDecimals(1000, 18));

    expect(await bonusTndTracker.stakedAmounts(user0.address)).eq(0);
    expect(
      await bonusTndTracker.depositBalances(
        user0.address,
        stakedTndTracker.address
      )
    ).eq(0);
    expect(await bonusTndTracker.stakedAmounts(user1.address)).eq(
      expandDecimals(1000, 18)
    );
    expect(
      await bonusTndTracker.depositBalances(
    ...
    expect(
      await feeTndTracker.depositBalances(
        user0.address,
        bonusTndTracker.address
      )
    ).eq(0);
    expect(await feeTndTracker.stakedAmounts(user1.address)).eq(
      expandDecimals(1000, 18)
    );
    expect(
      await feeTndTracker.depositBalances(
    ...
    expect(await bonusTndTracker.claimable(user1.address)).gt(
      "2730000000000000000"
    ); // 2.73, 1000 / 365 => ~2.74
    expect(await bonusTndTracker.claimable(user1.address)).lt(
      "2750000000000000000"
    ); // 2.75

    expect(await feeTndTracker.claimable(user0.address)).eq(0);
    expect(await feeTndTracker.claimable(user1.address)).gt(
      "3560000000000000000"
    ); // 3.56, 100 / 28 => ~3.57
    expect(await feeTndTracker.claimable(user1.address)).lt(
      "3580000000000000000"
    ); // 3.58
    ...
    expect(await stakedTndTracker.stakedAmounts(user2.address)).eq(
      expandDecimals(500, 18)
    );
    expect(
      await stakedTndTracker.depositBalances(user2.address, esTnd.address)
    ...
    expect(await bonusTndTracker.stakedAmounts(user2.address)).eq(
      expandDecimals(500, 18)
    );
    expect(
      await bonusTndTracker.depositBalances(
    ...
    expect(await feeTndTracker.stakedAmounts(user2.address)).eq(
      expandDecimals(500, 18)
    );
    expect(
      await feeTndTracker.depositBalances(
    ...
    expect(await stakedTndTracker.claimable(user2.address)).gt(
      expandDecimals(595, 18)
    );
    expect(await stakedTndTracker.claimable(user2.address)).lt(
    ...
    expect(await bonusTndTracker.claimable(user2.address)).gt(
      "1360000000000000000"
    ); // 1.36, 500 / 365 => ~1.37
    expect(await bonusTndTracker.claimable(user2.address)).lt(
      "1380000000000000000"
    );
    ...
    expect(await feeTndTracker.claimable(user2.address)).gt(
      "1180000000000000000"
    ); // 1.18, 100 / 28 / 3 => ~1.19
    expect(await feeTndTracker.claimable(user2.address)).lt(
      "1200000000000000000"
    );

    expect(await esTnd.balanceOf(user1.address)).eq(0);
    await rewardRouter.connect(user1).claimEsTnd();
    expect(await esTnd.balanceOf(user1.address)).gt(
      expandDecimals(1785 + 1190, 18)
    );
    expect(await esTnd.balanceOf(user1.address)).lt(
      expandDecimals(1786 + 1191, 18)
    );
    ...
    expect(await stakedTndTracker.stakedAmounts(user1.address)).gt(
      expandDecimals(3643, 18)
    );
    expect(await stakedTndTracker.stakedAmounts(user1.address)).lt(
    ...
    expect(
      await stakedTndTracker.depositBalances(user1.address, esTnd.address)
    ).gt(expandDecimals(2643, 18));
    expect(
      await stakedTndTracker.depositBalances(user1.address, esTnd.address)
    ).lt(expandDecimals(2645, 18));

    expect(await bonusTndTracker.stakedAmounts(user1.address)).gt(
      expandDecimals(3643, 18)
    );
    expect(await bonusTndTracker.stakedAmounts(user1.address)).lt(
      expandDecimals(3645, 18)
    );

    expect(await feeTndTracker.stakedAmounts(user1.address)).gt(
      expandDecimals(3657, 18)
    );
    expect(await feeTndTracker.stakedAmounts(user1.address)).lt(
    ...
    expect(
      await feeTndTracker.depositBalances(user1.address, bnTnd.address)
    ...
    const esTndUnstakeBalance1 = await stakedTndTracker.depositBalances(
      user1.address,
      esTnd.address
    );
    await rewardRouter.connect(user1).unstakeEsTnd(esTndUnstakeBalance1);
    expect(await esTnd.balanceOf(user1.address)).eq(
      esTndBalance1.add(esTndUnstakeBalance1)
    );
    ...
    await expect(
      rewardRouter.connect(user1).unstakeEsTnd(expandDecimals(1, 18))
  ...
  it("tnd: signalTransfer, acceptTransfer", async () => {
    ...
    await tnd
      .connect(user2)
      .approve(stakedTndTracker.address, expandDecimals(400, 18));
    ...
    await expect(
      ...
      "RewardRouter: stakedTndTracker.averageStakedAmounts > 0"
    );
    ...
    expect(
      await stakedTndTracker.depositBalances(user2.address, tnd.address)
    ).eq(expandDecimals(200, 18));
    expect(
      await stakedTndTracker.depositBalances(user2.address, esTnd.address)
    ).eq(0);
    expect(
      await feeTndTracker.depositBalances(user2.address, bnTnd.address)
    ).eq(0);
    expect(
      await stakedTndTracker.depositBalances(user3.address, tnd.address)
    ).eq(0);
    expect(
      await stakedTndTracker.depositBalances(user3.address, esTnd.address)
    ).eq(0);
    expect(
      await feeTndTracker.depositBalances(user3.address, bnTnd.address)
    ).eq(0);
    expect(await tndVester.transferredAverageStakedAmounts(user3.address)).eq(
      0
    );
    expect(await tndVester.transferredCumulativeRewards(user3.address)).eq(0);
    expect(await tndVester.bonusRewards(user2.address)).eq(
    ...
    expect(await tndVester.getMaxVestableAmount(user2.address)).eq(
      expandDecimals(100, 18)
    );
    expect(await tndVester.getMaxVestableAmount(user3.address)).eq(0);
    expect(
      await tndVester.getPairAmount(user2.address, expandDecimals(892, 18))
    ).eq(0);
    expect(
      await tndVester.getPairAmount(user3.address, expandDecimals(892, 18))
    ).eq(0);
    ...
    expect(await tndVester.transferredCumulativeRewards(user3.address)).gt(
      expandDecimals(892, 18)
    );
    expect(await tndVester.transferredCumulativeRewards(user3.address)).lt(
      expandDecimals(893, 18)
    );
    expect(await tndVester.bonusRewards(user2.address)).eq(0);
    expect(await tndVester.bonusRewards(user3.address)).eq(
      expandDecimals(100, 18)
    );
    expect(await tndVester.getCombinedAverageStakedAmount(user2.address)).eq(
      expandDecimals(200, 18)
    );
    expect(await tndVester.getCombinedAverageStakedAmount(user3.address)).eq(
      expandDecimals(200, 18)
    );
    expect(await tndVester.getMaxVestableAmount(user2.address)).eq(0);
    expect(await tndVester.getMaxVestableAmount(user3.address)).gt(
      expandDecimals(992, 18)
    );
    expect(await tndVester.getMaxVestableAmount(user3.address)).lt(
      expandDecimals(993, 18)
    );
    expect(
      await tndVester.getPairAmount(user2.address, expandDecimals(992, 18))
    ).eq(0);
    expect(
      await tndVester.getPairAmount(user3.address, expandDecimals(992, 18))
    ...
    expect(
      await stakedTndTracker.depositBalances(user4.address, tnd.address)
    ).eq(expandDecimals(200, 18));
    expect(
      await stakedTndTracker.depositBalances(user4.address, esTnd.address)
    ).gt(expandDecimals(892, 18));
    expect(
      await stakedTndTracker.depositBalances(user4.address, esTnd.address)
    ).lt(expandDecimals(893, 18));
    expect(
      await feeTndTracker.depositBalances(user4.address, bnTnd.address)
    ).gt("547000000000000000"); // 0.547
    expect(
      await feeTndTracker.depositBalances(user4.address, bnTnd.address)
    ).lt("549000000000000000"); // 0.548
    expect(await tndVester.transferredAverageStakedAmounts(user4.address)).gt(
      expandDecimals(200, 18)
    );
    expect(await tndVester.transferredAverageStakedAmounts(user4.address)).lt(
      expandDecimals(201, 18)
    );
    expect(await tndVester.transferredCumulativeRewards(user4.address)).gt(
      expandDecimals(892, 18)
    );
    expect(await tndVester.transferredCumulativeRewards(user4.address)).lt(
      expandDecimals(894, 18)
    );
    expect(await tndVester.bonusRewards(user3.address)).eq(0);
    expect(await tndVester.bonusRewards(user4.address)).eq(
      expandDecimals(100, 18)
    );
    expect(await stakedTndTracker.averageStakedAmounts(user3.address)).gt(
      expandDecimals(1092, 18)
    );
    expect(await stakedTndTracker.averageStakedAmounts(user3.address)).lt(
      expandDecimals(1094, 18)
    );
    expect(await tndVester.transferredAverageStakedAmounts(user3.address)).eq(
      0
    );
    expect(await tndVester.getCombinedAverageStakedAmount(user3.address)).gt(
      expandDecimals(1092, 18)
    );
    expect(await tndVester.getCombinedAverageStakedAmount(user3.address)).lt(
      expandDecimals(1094, 18)
    );
    expect(await tndVester.getCombinedAverageStakedAmount(user4.address)).gt(
      expandDecimals(200, 18)
    );
    expect(await tndVester.getCombinedAverageStakedAmount(user4.address)).lt(
      expandDecimals(201, 18)
    );
    expect(await tndVester.getMaxVestableAmount(user3.address)).eq(0);
    expect(await tndVester.getMaxVestableAmount(user4.address)).gt(
      expandDecimals(992, 18)
    );
    expect(await tndVester.getMaxVestableAmount(user4.address)).lt(
      expandDecimals(993, 18)
    );
    expect(
      await tndVester.getPairAmount(user3.address, expandDecimals(992, 18))
    ...
    expect(
      await tndVester.getPairAmount(user4.address, expandDecimals(992, 18))
    ).gt(expandDecimals(199, 18));
    expect(
      await tndVester.getPairAmount(user4.address, expandDecimals(992, 18))
    ).lt(expandDecimals(200, 18));
  ...
  it("handleRewards", async () => {
    ...
    expect(
      await stakedTndTracker.depositBalances(user1.address, esTnd.address)
    ).gt(expandDecimals(1785, 18)); // 50000 / 28 => ~1785
    expect(
      await stakedTndTracker.depositBalances(user1.address, esTnd.address)
    ).lt(expandDecimals(1786, 18));
    expect(
      await feeTndTracker.depositBalances(user1.address, bnTnd.address)
    ).gt("540000000000000000"); // 200 / 365 => ~0.55
    expect(
      await feeTndTracker.depositBalances(user1.address, bnTnd.address)
    ...
    expect(
      await feeTndTracker.depositBalances(user1.address, bnTnd.address)
    ).gt("540000000000000000");
    expect(
      await feeTndTracker.depositBalances(user1.address, bnTnd.address)
    ).lt("560000000000000000");
    ...
    expect(await tnd.balanceOf(user1.address)).eq(0);
    expect(await esTnd.balanceOf(user1.address)).gt(
      expandDecimals(1785 - 365, 18)
    );
    expect(await esTnd.balanceOf(user1.address)).lt(
      expandDecimals(1786 - 365, 18)
    );
    expect(await bnTnd.balanceOf(user1.address)).eq(0);
    expect(await eth.balanceOf(user1.address)).gt("3560000000000000000");
    expect(await eth.balanceOf(user1.address)).lt("3580000000000000000");
