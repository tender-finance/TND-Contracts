...
describe("BonusDistributor", function () {
  ...
  before(async () => {
    ...
    [wallet, rewardRouter, user0, user1, user2, user3] =
  ...
  it("distributes bonus", async () => {
    ...
    await tnd
      .connect(user0)
      .approve(stakedTndTracker.address, expandDecimals(1001, 18));
    await expect(
      stakedTndTracker
    ...
    expect(await bonusTndTracker.claimable(user0.address)).gt(
      "2730000000000000000"
    ); // 2.73, 1000 / 365 => ~2.74
    expect(await bonusTndTracker.claimable(user0.address)).lt(
      "2750000000000000000"
    ); // 2.75
