...
describe("Vester", function () {
  ...
  it("deposit, claim, withdraw", async () => {
    ...
    await esTnd.connect(user0).approve(vester.address, expandDecimals(1000, 18))
    ...
    expect(await vester.balanceOf(user0.address)).eq(expandDecimals(1000, 18).sub(tndAmount))
  ...
  it("handles pairing", async () => {
    ...
    await tnd.connect(user1).approve(stakedTndTracker.address, expandDecimals(500, 18))
    ...
    expect(await stakedTndTracker.claimable(user0.address)).gt(expandDecimals(1190, 18))
    expect(await stakedTndTracker.claimable(user0.address)).lt(expandDecimals(1191, 18))
    expect(await stakedTndTracker.claimable(user1.address)).gt(expandDecimals(594, 18))
    ...
    await tnd.connect(user0).approve(stakedTndTracker.address, expandDecimals(500, 18))
    ...
    expect(await feeTndTracker.balanceOf(user0.address)).eq(expandDecimals(1500, 18))
    ...
    expect(await feeTndTracker.balanceOf(user0.address)).gt(expandDecimals(499, 18))
    expect(await feeTndTracker.balanceOf(user0.address)).lt(expandDecimals(501, 18))
    expect(await feeTndTracker.balanceOf(vester.address)).gt(expandDecimals(999, 18))
    expect(await feeTndTracker.balanceOf(vester.address)).lt(expandDecimals(1001, 18))
  ...
  it("handles existing pair tokens", async () => {
    ...
    expect(await feeTndTracker.balanceOf(user0.address)).eq(expandDecimals(1000, 18))
    await esTnd.mint(user0.address, expandDecimals(2380, 18))
    await vester.connect(user0).deposit(expandDecimals(2380, 18))

    expect(await feeTndTracker.balanceOf(user0.address)).gt(0)
    expect(await feeTndTracker.balanceOf(user0.address)).lt(expandDecimals(1, 18))
    ...
    expect(feeTndTrackerBalance).eq(await feeTndTracker.balanceOf(user0.address))

    await expect(rewardRouter.connect(user0).unstakeTnd(expandDecimals(2, 18)))
