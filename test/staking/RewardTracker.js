...
describe("RewardTracker", function () {
  ...
  beforeEach(async () => {
    ...
    rewardDistributor = await deployContract("RewardDistributor", [esTnd.address, rewardTracker.address])
    await rewardDistributor.updateLastDistributionTime()

    await rewardTracker.initialize([tnd.address, esTnd.address], rewardDistributor.address)
  })

  it("inits", async () => {
    expect(await rewardTracker.isInitialized()).eq(true)
    expect(await rewardTracker.isDepositToken(wallet.address)).eq(false)
    expect(await rewardTracker.isDepositToken(tnd.address)).eq(true)
    expect(await rewardTracker.isDepositToken(esTnd.address)).eq(true)
    expect(await rewardTracker.distributor()).eq(rewardDistributor.address)
    expect(await rewardTracker.distributor()).eq(rewardDistributor.address)
    expect(await rewardTracker.rewardToken()).eq(esTnd.address)

    await expect(rewardTracker.initialize([tnd.address, esTnd.address], rewardDistributor.address))
  ...
  it("withdrawToken", async () => {
    ...
    await tnd.mint(rewardTracker.address, 2000)
    await expect(rewardTracker.connect(user0).withdrawToken(tnd.address, user1.address, 2000))
      .to.be.revertedWith("Governable: forbidden")

    await rewardTracker.setGov(user0.address)

    expect(await tnd.balanceOf(user1.address)).eq(0)
    await rewardTracker.connect(user0).withdrawToken(tnd.address, user1.address, 2000)
    expect(await tnd.balanceOf(user1.address)).eq(2000)
  })

  it("stake, unstake, claim", async () => {
    await esTnd.setMinter(wallet.address, true)
    await esTnd.mint(rewardDistributor.address, expandDecimals(50000, 18))
    await rewardDistributor.setTokensPerInterval("20667989410000000") // 0.02066798941 esTnd per second
    await tnd.setMinter(wallet.address, true)
    await tnd.mint(user0.address, expandDecimals(1000, 18))

    await rewardTracker.setInPrivateStakingMode(true)
    await expect(rewardTracker.connect(user0).stake(tnd.address, expandDecimals(1000, 18)))
    ...
    await tnd.connect(user0).approve(rewardTracker.address, expandDecimals(1000, 18))
    await rewardTracker.connect(user0).stake(tnd.address, expandDecimals(1000, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, tnd.address)).eq(expandDecimals(1000, 18))
    ...
    await esTnd.connect(user1).approve(rewardTracker.address, expandDecimals(500, 18))
    await rewardTracker.connect(user1).stake(esTnd.address, expandDecimals(500, 18))
    expect(await rewardTracker.stakedAmounts(user1.address)).eq(expandDecimals(500, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, tnd.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, esTnd.address)).eq(0)
    expect(await rewardTracker.depositBalances(user1.address, tnd.address)).eq(0)
    expect(await rewardTracker.depositBalances(user1.address, esTnd.address)).eq(expandDecimals(500, 18))
    expect(await rewardTracker.totalDepositSupply(tnd.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.totalDepositSupply(esTnd.address)).eq(expandDecimals(500, 18))
    ...
    await expect(rewardTracker.connect(user0).unstake(esTnd.address, expandDecimals(1001, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    await expect(rewardTracker.connect(user0).unstake(esTnd.address, expandDecimals(1000, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");

    await expect(rewardTracker.connect(user0).unstake(tnd.address, expandDecimals(1001, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    expect(await tnd.balanceOf(user0.address)).eq(0)
    await rewardTracker.connect(user0).unstake(tnd.address, expandDecimals(1000, 18))
    expect(await tnd.balanceOf(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.totalDepositSupply(tnd.address)).eq(0)
    expect(await rewardTracker.totalDepositSupply(esTnd.address)).eq(expandDecimals(500, 18))
    ...
    await expect(rewardTracker.connect(user0).unstake(tnd.address, 1))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    expect(await esTnd.balanceOf(user0.address)).eq(0)
    await rewardTracker.connect(user0).claim(user2.address)
    expect(await esTnd.balanceOf(user2.address)).gt(expandDecimals(1785 + 1190, 18))
    expect(await esTnd.balanceOf(user2.address)).lt(expandDecimals(1786 + 1191, 18))
    ...
    await tnd.mint(user1.address, expandDecimals(300, 18))
    await tnd.connect(user1).approve(rewardTracker.address, expandDecimals(300, 18))
    await rewardTracker.connect(user1).stake(tnd.address, expandDecimals(300, 18))
    expect(await rewardTracker.totalDepositSupply(tnd.address)).eq(expandDecimals(300, 18))
    expect(await rewardTracker.totalDepositSupply(esTnd.address)).eq(expandDecimals(500, 18))
    ...
    await expect(rewardTracker.connect(user1).unstake(tnd.address, expandDecimals(301, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");

    await expect(rewardTracker.connect(user1).unstake(esTnd.address, expandDecimals(501, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");
    ...
    expect(await esTnd.balanceOf(user2.address)).eq(await rewardTracker.cumulativeRewards(user0.address))
    expect(await esTnd.balanceOf(user3.address)).eq(await rewardTracker.cumulativeRewards(user1.address))

    expect(await tnd.balanceOf(user1.address)).eq(0)
    expect(await esTnd.balanceOf(user1.address)).eq(0)
    await rewardTracker.connect(user1).unstake(tnd.address, expandDecimals(300, 18))
    expect(await tnd.balanceOf(user1.address)).eq(expandDecimals(300, 18))
    expect(await esTnd.balanceOf(user1.address)).eq(0)
    await rewardTracker.connect(user1).unstake(esTnd.address, expandDecimals(500, 18))
    expect(await tnd.balanceOf(user1.address)).eq(expandDecimals(300, 18))
    expect(await esTnd.balanceOf(user1.address)).eq(expandDecimals(500, 18))
    expect(await rewardTracker.totalDepositSupply(tnd.address)).eq(0)
    expect(await rewardTracker.totalDepositSupply(esTnd.address)).eq(0)

    await rewardTracker.connect(user0).claim(user2.address)
    await rewardTracker.connect(user1).claim(user3.address)

    const distributed = expandDecimals(50000, 18).sub(await esTnd.balanceOf(rewardDistributor.address))
  ...
  it("stakeForAccount, unstakeForAccount, claimForAccount", async () => {
    ...
    await rewardDistributor.setTokensPerInterval("20667989410000000") // 0.02066798941 esTnd per second
    await tnd.setMinter(wallet.address, true)
    await tnd.mint(wallet.address, expandDecimals(1000, 18))

    await rewardTracker.setInPrivateStakingMode(true)
    await expect(rewardTracker.connect(user0).stake(tnd.address, expandDecimals(1000, 18)))
      .to.be.revertedWith("RewardTracker: action not enabled")

    await expect(rewardTracker.connect(user2).stakeForAccount(wallet.address, user0.address, tnd.address, expandDecimals(1000, 18)))
    ...
    await tnd.connect(wallet).approve(rewardTracker.address, expandDecimals(1000, 18))

    await rewardTracker.connect(user2).stakeForAccount(wallet.address, user0.address, tnd.address, expandDecimals(1000, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, tnd.address)).eq(expandDecimals(1000, 18))
    ...
    await expect(rewardTracker.connect(user2).unstakeForAccount(user0.address, esTnd.address, expandDecimals(1000, 18), user1.address))
      .to.be.revertedWith("RewardTracker: forbidden")
    ...
    await expect(rewardTracker.connect(user2).unstakeForAccount(user0.address, tnd.address, expandDecimals(1001, 18), user1.address))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount")
    ...
    await rewardTracker.connect(user2).unstakeForAccount(user0.address, tnd.address, expandDecimals(100, 18), user1.address)

    expect(await tnd.balanceOf(user1.address)).eq(expandDecimals(100, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(900, 18))
    expect(await rewardTracker.depositBalances(user0.address, tnd.address)).eq(expandDecimals(900, 18))
