...
describe("RewardTracker", function () {
  ...
  before(async () => {
    ...
    [wallet, user0, user1, user2, user3] = await hre.ethers.getSigners();
  });
  ...
  it("withdrawToken", async () => {
    ...
    await expect(
      rewardTracker
        .connect(user0)
        .withdrawToken(tnd.address, user1.address, 2000)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  it("stake, unstake, claim", async () => {
    ...
    await expect(
      rewardTracker.connect(user0).stake(tnd.address, expandDecimals(1000, 18))
    ...
    await rewardTracker
      .connect(user0)
      .stake(tnd.address, expandDecimals(1000, 18));
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(
      expandDecimals(1000, 18)
    );
    expect(await rewardTracker.depositBalances(user0.address, tnd.address)).eq(
      expandDecimals(1000, 18)
    );
    ...
    await rewardTracker
      .connect(user1)
      .stake(esTnd.address, expandDecimals(500, 18));
    ...
    expect(
      await rewardTracker.depositBalances(user0.address, esTnd.address)
    ).eq(0);
    expect(await rewardTracker.depositBalances(user1.address, tnd.address)).eq(
      0
    );
    expect(
      await rewardTracker.depositBalances(user1.address, esTnd.address)
    ).eq(expandDecimals(500, 18));
    expect(await rewardTracker.totalDepositSupply(tnd.address)).eq(
      expandDecimals(1000, 18)
    );
    expect(await rewardTracker.totalDepositSupply(esTnd.address)).eq(
      expandDecimals(500, 18)
    );
    ...
    await expect(
      rewardTracker
        .connect(user0)
        .unstake(esTnd.address, expandDecimals(1001, 18))
    ).to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    await expect(
      rewardTracker
        .connect(user0)
        .unstake(esTnd.address, expandDecimals(1000, 18))
    ).to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");

    await expect(
      rewardTracker
        .connect(user0)
        .unstake(tnd.address, expandDecimals(1001, 18))
    ).to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    expect(await tnd.balanceOf(user0.address)).eq(0);
    await rewardTracker
      .connect(user0)
      .unstake(tnd.address, expandDecimals(1000, 18));
    ...
    await expect(
      rewardTracker.connect(user0).unstake(tnd.address, 1)
    ).to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    expect(await esTnd.balanceOf(user0.address)).eq(0);
    await rewardTracker.connect(user0).claim(user2.address);
    expect(await esTnd.balanceOf(user2.address)).gt(
      expandDecimals(1785 + 1190, 18)
    );
    expect(await esTnd.balanceOf(user2.address)).lt(
      expandDecimals(1786 + 1191, 18)
    );
    ...
    await rewardTracker
      .connect(user1)
      .stake(tnd.address, expandDecimals(300, 18));
    expect(await rewardTracker.totalDepositSupply(tnd.address)).eq(
    ...
    await expect(
      rewardTracker.connect(user1).unstake(tnd.address, expandDecimals(301, 18))
    ).to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");

    await expect(
      rewardTracker
        .connect(user1)
        .unstake(esTnd.address, expandDecimals(501, 18))
    ).to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");
    ...
    expect(await esTnd.balanceOf(user2.address)).eq(
      await rewardTracker.cumulativeRewards(user0.address)
    );
    expect(await esTnd.balanceOf(user3.address)).eq(
    ...
    await rewardTracker
      .connect(user1)
      .unstake(tnd.address, expandDecimals(300, 18));
    expect(await tnd.balanceOf(user1.address)).eq(expandDecimals(300, 18));
    expect(await esTnd.balanceOf(user1.address)).eq(0);
    await rewardTracker
      .connect(user1)
      .unstake(esTnd.address, expandDecimals(500, 18));
    expect(await tnd.balanceOf(user1.address)).eq(expandDecimals(300, 18));
    ...
    const distributed = expandDecimals(50000, 18).sub(
      await esTnd.balanceOf(rewardDistributor.address)
    );
