...
async function main() {
  ...
  const tndToVester = expandDecimals(2 * 1000 * 1000, 18)          // 2 million
  const tndToOwnerWallet = expandDecimals(8 * 1000 * 1000, 18)     // 8 million
  ...
  await sendTxn(tnd.mint(tndVester.address, tndToVester), "tnd.mint(tndVester")
  await sendTxn(tnd.mint(wallet.address, tndToOwnerWallet), "tnd.mint(wallet")
